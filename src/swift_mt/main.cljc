(ns swift-mt.main
  "Kotodama WASM entrypoint for the swift_mt clean-room actor (L4) — Clojure port.
  ...
  facts (`swift_mt.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "swift_mt")
(def tier "L4")
...
(def entity-specs
  [{:entity "TradeItem" :plural "tradeitems" :id-prefix "swiftmt_tra"
    :fields [:gtin :description :brand]              :required [:gtin :description]
    :coerce {}                       :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "swiftmt_eve"
    :fields [:itemId :bizStep :disposition :recordedAt] :required [:bizStep :disposition]
    :coerce {}                       :refs {}}
   {:entity "Document"  :plural "documents"  :id-prefix "swiftmt_doc"
    :fields [:type :senderId :receiverId :status]   :required [:type :status]
    :coerce {}                       :refs {}}
   {:entity "Container" :plural "containers" :id-prefix "swiftmt_con"
    :fields [:bicCode :type :status]                 :required [:bicCode :type]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "swiftmt_par"
    :fields [:gln :name :role]                       :required [:gln :name]
    :coerce {}                       :refs {}}
   {:entity "Shipment"  :plural "shipments"  :id-prefix "swiftmt_shi"
...
(defn emit-facts
  "EAVT facts for one record: {\"swift_mt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "swift_mt-compat" :tier tier :entities entities} 200])
...
(defn start! [] :swift_mt-compat/ready)
